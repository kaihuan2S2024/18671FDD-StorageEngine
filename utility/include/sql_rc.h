 ...
 *
 *
 * Note that in the Sqlite's original implementation, the result code is a 32-bit integer
 * The last 8 bits are the primary result code, and the first 24 bits used to extended result code.
 * SQLITE_ERROR_SNAPSHOT is SQLITE_ERROR | (3<<8), where SQLITE_ERROR is the value 1.
 * This allows the primary result code to be contained within the full result code.
 *
 * Many of Sqlite's functions return a result code
 * By returning result codes, we can avoid throwing exceptions
 * Google's C++ Style Guide recommends using exceptions only for exceptional cases
 ...
 * Returns the name for the primary result code, or "" if it is an unknown value.
 */
